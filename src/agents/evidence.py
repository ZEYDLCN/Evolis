...
def build_evidence(analysis: dict) -> dict:
    ...
    for topic, score in list(interests.items())[:3]:
        bullets.append(f"{topic} interest score: {round(score * 100)}%")

    skills: list = analysis.get("skills") or []
    for skill in skills[:2]:
        bullets.append(f"{skill['skill']} skill activity: {skill['activity_score']}/100")

    source_entries = [hit["text"] for hit in (analysis.get("retrieved_entries") or [])]
    if source_entries and not bullets:
        bullets.append(f"Found {len(source_entries)} related past entries")
